...
### 화면 레이아웃 (사이드바 셸)
- 최상단 네비게이션은 `components/Sidebar.tsx` 하나다(과거 `Header.tsx`는 삭제됨). 데스크톱(`md:` 이상)은 좌측 고정 세로 사이드바, 그 아래는 상단 가로 바로 접힌다. 새 탭을 추가할 때는 이 파일의 `TABS` 배열에 추가하면 되고, `/login`/`/forgot-password`/`/reset-password`/`/preview*`에서는 자동으로 숨겨진다(해당 로직 그대로 유지).
