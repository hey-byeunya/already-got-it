    ...
    <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            className={`rounded-xl border-[1.5px] px-4 py-2.5 text-sm font-medium transition-colors ${
              active
                ...
                : 'border-surface-border bg-input-bg text-muted hover:border-accent/40'
            }`}
          >
            {category}
          </button>
      ...
      {addingNew ? (
        <input
          ...
          placeholder="카테고리 이름"
          className="rounded-xl border-[1.5px] border-accent bg-input-bg px-4 py-2.5 text-sm outline-none"
        />
      ) : (
        <button
          type="button"
          onClick={() => setAddingNew(true)}
          className="flex items-center gap-1 rounded-xl border-[1.5px] border-dashed border-surface-border bg-input-bg px-4 py-2.5 text-sm font-medium text-muted transition-colors hover:border-accent/40"
        >
