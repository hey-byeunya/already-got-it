    ...
    <div className="flex flex-wrap items-center gap-2">
      {collapsedOptions.map((category) => {
        ...
        return (
          <button
            ...
            className={`whitespace-nowrap rounded-full border-[1.5px] px-3.5 py-1.5 text-sm font-medium shadow-sm transition-colors ${
              active
                ? 'border-accent bg-accent-soft font-bold text-accent-hover'
                : 'border-surface-border bg-surface text-muted hover:border-accent/40'
            }`}
          >
      ...
      {hasMore && (
        <button
          ...
          className="flex items-center gap-1 whitespace-nowrap rounded-full border-[1.5px] border-dashed border-surface-border bg-surface px-3.5 py-1.5 text-sm font-medium text-muted shadow-sm transition-colors hover:border-accent/40"
        >
          더보기
