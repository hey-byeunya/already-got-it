...
- 회원가입 직후 로그인 화면으로 한 번만 이동하는지(사이드바가 잠깐이라도 보이지 않는지), 가입 확인 안내 배너가 뜨고 이메일 입력칸이 방금 가입한 주소로 채워져 있는지
...
- 존재하지 않는 보유템/위시 ID로 수정 화면을 열어 에러 화면("문제가 발생했어요")이 뜨는지, "다시 시도"/"홈으로"가 동작하는지 확인
- 에러 발생 후 Supabase 대시보드 `error_logs`에 본인 `user_id`로 message+route 행이 1건 기록됐는지, 다른 계정으로 로그인하면 해당 행이 보이지 않는지 확인
